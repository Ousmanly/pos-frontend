<template>
    <div class="container">
      ...
      <!-- <h1>{{ `size is ${size}` }}</h1> -->
        ...
        <div class="mb-3">
          ...
          <div v-if="errors.name" class="text-danger mt-1">{{ errors.name }}</div>        </div>
        <div class="mb-3">
          ...
          <input
            ...
            required
          />
          <div v-if="errors.phone" class="text-danger mt-1">{{ errors.phone }}</div>        </div>
  ...
  <script setup>
    ...
    try {
        ...
        // toast.error("faild to create a supplier")
    ...
    //   }
    if (error.response && error.response.status === 422) {
    const apiErrors = error.response.data.errors; // Le tableau d'erreurs
    apiErrors.forEach((err) => {
        // Utilisez `err.param` pour identifier le champ et `err.msg` pour le message d'erreur
